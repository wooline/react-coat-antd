...
class ModuleHandlers extends BaseModuleHandlers<ModuleState, RootState, ModuleActions> {
  ...
  @reducer
  setActivedTab(activedTabNavId: string): ModuleState {
    return { ...this.state, activedTabNavId, curTabNav: null };
  }
  @reducer
  setCurTabNav(curTabNav: TabNav): ModuleState {
  ...
  *activeTabNav(item: TabNav): SagaIterator {
    if (item && this.state.activedTabNavId !== item.id) {
      ...
      yield this.put(this.actions.setCurTabNav(item));
    }
  }
  @globalLoading
  @effect
  *START(): SagaIterator {
    ...
    yield this.put(this.actions.STARTED({ ...this.state, layout, tabNavs, tabNavsMap: arrayToMap(tabNavs) }));
  }
  @effect
  protected *[LOCATION_CHANGE as string](payload: RouterState): SagaIterator {
    const url = getNewTabNavUrl(payload);
    yield this.put(this.actions.setActivedTab(url));
  }
}
